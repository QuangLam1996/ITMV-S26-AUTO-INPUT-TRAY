...
'		Jump PCB_Pick_Wait_Pos limz 0 CP
		Jump Homp LimZ -10; Off Out_Robot_1_Interlock
        
		
	ElseIf CX(PCB_Pick_Wait_Pos) + 150 < CX(Here) Then
		Jump Panel_Enter CP
		Jump Homp; Off Out_Robot_1_Interlock
		
	ElseIf CX(PCB_Pick_Wait_Pos) + 150 >= CX(Here) And CX(PCB_Place_Wait_Pos) + 100 < CX(Here) Then
		'Jump PCB_Pick_Wait_Pos_Even LimZ 0 CP
		'Jump PCB_Pick_Wait_Pos
		Move PCB_Pick_Wait_Pos
		Jump Homp LimZ -10; Off Out_Robot_1_Interlock
	
	ElseIf CX(PCB_Place_Wait_Pos) + 100 >= CX(Here) Then
		'Jump PCB_Place_Wait_Pos LimZ 0 CP
...
Function SIG
	...
	Off Out_Robot_1_Interlock; Off Robot_Busy; Out 65, 0; Out 66, 0; Out 67, 0; Out 68, 0; Out 69, 0; Out 70, 0; Out 71, 0; BypassRotate = 0; Off PCB_Place_InPos; Off PCB_Place_Re; Off PCB_Place_Com
	'Error 8010
...
Function Pick_Panel_Wait_Move
	...
	Call Low_Speed
	On Robot_Busy; On Out_Robot_1_Interlock; Wait Sw(In_Robot_2_Interlock) = Off
'	On Pick_Panel_Wait_Re, 0.1
	
	If CX(PCB_Pick_Wait_Pos) + 150 < CX(Here) Then
		Wait Sw(In_Robot_2_Interlock) = Off
...
Function Place_Panel_Wait_Move
	...
	If CX(PCB_Pick_Wait_Pos) + 150 < CX(Here) Then
		...
		Jump Panel_Enter CP
		Jump PCB_Pick_Wait_Pos CP
		Off Out_Robot_1_Interlock

	ElseIf CX(PCB_Pick_Wait_Pos) + 150 >= CX(Here) And CX(PCB_Place_Wait_Pos) + 100 < CX(Here) Then
		Jump Place_Panel_Wait_Pos LimZ -10 CP

	ElseIf CX(PCB_Place_Wait_Pos) + 100 >= CX(Here) Then
		Jump PCB_Place_Wait_Pos LimZ -10 CP

	EndIf

	Jump Place_Panel_Wait_Pos LimZ -10
...
Function Place_Panel_Move
	...
	If CX(PCB_Pick_Wait_Pos) + 150 < CX(Here) Then
		Wait Sw(In_Robot_2_Interlock) = Off
		Jump Panel_Enter LimZ 0 CP
		Jump PCB_Pick_Wait_Pos LimZ 0 CP
		Off Out_Robot_1_Interlock
		
	ElseIf CX(PCB_Pick_Wait_Pos) + 150 + 100 >= CX(Here) And CX(PCB_Place_Wait_Pos) < CX(Here) Then
		Jump Place_Panel_Wait_Pos LimZ -10 CP
...
Recycle_re:
	...
	Print "Remove_Recycle_Wait Move !"
	Call Low_Speed
	On Robot_Busy
	...
	ElseIf CX(PCB_Pick_Wait_Pos) >= CX(Here) And CX(PCB_Place_Wait_Pos) < CX(Here) Then
		Jump Place_Panel_Wait_Pos LimZ -10 CP
	
	ElseIf CX(PCB_Place_Wait_Pos) >= CX(Here) Then
		Jump PCB_Place_Wait_Pos LimZ -10 CP
		Jump Place_Panel_Wait_Pos LimZ -10 CP
	...
	Jump Place_Panel_Pos :Z(0) LimZ 0
		
	If Sw(Err_Vaccum_Frame) = On Then
		
		Jump Place_Panel_Pos :Z(0) LimZ 0
				
		Wait Sw(Pick_Again_Frame) = On; GoTo Recycle_Re
...
Function Remove_Recycle_Move
	...
	ElseIf CX(PCB_Pick_Wait_Pos) >= CX(Here) And CX(PCB_Place_Wait_Pos) < CX(Here) Then
'		Jump PCB_Pick_Wait_Pos :z(0) limz 0 CP
		Wait Sw(In_Robot_2_Interlock) = Off
		Jump Panel_Enter LimZ 0 CP
	
	ElseIf CX(PCB_Place_Wait_Pos) >= CX(Here) Then
		Jump PCB_Place_Wait_Pos :Z(0) LimZ 0 CP
		Jump PCB_Pick_Wait_Pos :Z(0) LimZ 0 CP
		Wait Sw(In_Robot_2_Interlock) = Off
		Jump Panel_Enter LimZ 0 CP
	...
	Jump Remove_Recycle_Pos LimZ 0
...
Function PCB_Pick_Wait_Move
	...
	If Out(Pos_Inpos) = 6 Then
	    Print "PCB_Pick_Wait_Move is here!!! Out(Pos_Inpos) = 6"
		GoTo Pick_End
	EndIf
...
'		EndIf
		...
		Off Out_Robot_1_Interlock;
		GoTo Pick_End
	ElseIf CX(PCB_Pick_Wait_Pos) + 150 >= CX(Here) And CX(PCB_Place_Wait_Pos) < CX(Here) Then
		Jump PCB_Pick_Wait_Pos LimZ -10 CP
'        If (Pallet_Pick_Number_In = 1) Then
'			Jump PCB_Pick_Wait_Pos LimZ -10 CP
'		EndIf
'		If (Pallet_Pick_Number_In = 2) Then
'			Jump PCB_Pick_Wait_Pos_Even LimZ -10 CP
'		EndIf
		GoTo Pick_End
...
Pick_End:
	...
	Off Robot_Busy; Off Out_Robot_1_Interlock;
...
Function PCB_Pick_Move
	...
	EndIf
	'add bypass rotate last round
	'Jump Pallet(Pallet_Pick_Number_In, Pick_Number_In) LimZ -10 ! D80; On PCB_Pick_InPos, 0.05 !
	'Jump Pallet(Pallet_Pick_Number_In, Pick_Number_In) LimZ -10 ! D80; On PCB_Pick_InPos !
...
NG_Next:
	...
	If NG_Pos_Num_In = 1 Or NG_Pos_Num_In = 4 Then
		Jump PCB_NG_Pos_1 LimZ -10
		
	ElseIf NG_Pos_Num_In = 2 Or NG_Pos_Num_In = 3 Then
		Jump PCB_NG_Pos_2 LimZ -10
	EndIf
...
Function PCB_Place_Wait_Move
	...
	ElseIf CX(PCB_Pick_Wait_Pos) + 150 >= CX(Here) And CX(PCB_Place_Wait_Pos) < CX(Here) Then
		If InsideBox(5, 1) = True Then
			Jump PCB_Place_Wait_Pos :Z(-10) LimZ CZ(PCB_NG_Pos_1) CP
		Else
...
'		EndIf
			...
			Jump PCB_Pick_Wait_Pos LimZ -10 CP
			Jump PCB_Place_Wait_Pos :Z(-10) LimZ -10 CP
...
Function PCB_Place_Move
	...
	If CX(PCB_Pick_Wait_Pos) + 150 < CX(Here) Then
		...
		Jump Panel_Enter LimZ 0 CP
		Jump PCB_Pick_Wait_Pos LimZ 0 CP
		Jump PCB_Place_Wait_Pos :Z(-10) LimZ 0 CP
		Off Out_Robot_1_Interlock;

	ElseIf CX(PCB_Pick_Wait_Pos) + 150 >= CX(Here) And CX(PCB_Place_Wait_Pos) + 100 < CX(Here) Then
		If InsideBox(5, 1) = True Then
			Jump PCB_Place_Wait_Pos :Z(-10) LimZ CZ(PCB_NG_Pos_1)
		Else
...
'		    EndIf
			Jump PCB_Pick_Wait_Pos LimZ -10 CP
			Jump PCB_Place_Wait_Pos :Z(-10) LimZ -10
...
Function Model_Input
	...
	'Box Data
		...
		Box 3, CX(PCB_Pick_2_2) - 5, CX(PCB_Pick_1_2) + 5, CY(PCB_Pick_1_3) - 5, CY(PCB_Pick_2_1) + 5, CZ(PCB_Pick_1_1) - 2, CZ(PCB_Pick_Wait_Pos) + 2
...
Do
	TmReset 1
	Go PCB_Pick_Wait_Pos
	Go PCB_Place_Wait_Pos
	Print " TT ", Tmr(1)
Loop
Fend
...
Function Up_PCB_Pick_Move
	...
	SpeedS sp
	AccelS sp, sp
	'Wait Sw(Hand_Work_Com)
	Move Here :Z(CZ(PCB_Pick_Wait_Pos_Even))
	'Jump Pallet(Pallet_Pick_Number_In, Pick_Number_In) :Z(CZ(PCB_Pick_Wait_Pos_Even)) C0 LimZ -30
	
    If (Pallet_Pick_Number_In = 1) Then
		'Jump PCB_Pick_Wait_Pos C0 LimZ -30 CP ! D20; On PCB_Pick_Com, 0.05 !
'		Jump PCB_Pick_Wait_Pos C0 LimZ -30 CP ! D20; On PCB_Pick_Com !
'		Wait Sw(Hand_Work_Com) = Off;
'		Off PCB_Pick_Com
		Jump PCB_Pick_Wait_Pos C0 LimZ -10 CP
	EndIf
	If (Pallet_Pick_Number_In = 2) Then
		'Jump PCB_Pick_Wait_Pos_Even C0 LimZ -30 CP ! D20; On PCB_Pick_Com, 0.05 !
'		Jump PCB_Pick_Wait_Pos_Even C0 LimZ -30 CP ! D20; On PCB_Pick_Com !
'		Wait Sw(Hand_Work_Com) = Off;
'		Off PCB_Pick_Com
